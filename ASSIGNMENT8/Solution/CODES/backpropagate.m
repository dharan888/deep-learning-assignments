function network_synaptics = backpropagate(learning_rate,act_fn,network_synaptics,layers,errors)
    ...
    for s = no_of_synaptics:-1:1
        ...
        network_synaptics(s).weights = network_synaptics(s).weights+(learning_rate/m)*delta*transpose(layers(s).activations);
        network_synaptics(s).biases  = network_synaptics(s).biases+(learning_rate/m)*sum(delta,2);
    end
end